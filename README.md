...
> **ВАЖНО!** АКТИВНАЯ РАЗРАБОТКА ПРОЕКТА ПРЕКРАЩЕНА, ВОЗМОЖНО ТОЛЬКО ИСПРАВЛЕНИЕ ОБНАРУЖЕННЫХ ОШИБОК. СОСТОЯНИЕ СТАБИЛЬНОЕ, ИЗВЕСТНЫХ ОШИБОК НЕ ОБНАРУЖЕНО. ЗА НОВЫМ ПОКОЛЕНИЕМ ФРЕЙМВОРКА СТОИТ СЛЕДИТЬ [В БЛОГЕ](https://leopotam.com/).
...
# Лицензия
Фреймворк выпускается под [MIT-Red лицензией](./LICENSE.md).
